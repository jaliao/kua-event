...
schema-update: prisma-format prisma-validate prisma-migrate prisma-generate dev-restart ## 🚀 完整 Schema 更新流程（推薦）
	@echo "✅ Schema 更新完成！"
	@echo "💡 已執行："
	@echo "   1. 格式化 Schema"
	@echo "   2. 驗證 Schema"
...
schema-quick: prisma-push prisma-generate dev-restart ## ⚡ 快速更新（開發用，不建立 migration）
	@echo "✅ 快速更新完成！"
...
prisma-inspect: ## 🔬 檢查目前資料庫結構
	...
	@echo ""
	@echo "📋 所有資料表："
	@$(DEV_COMPOSE) exec db psql -U $(POSTGRES_USER) -d $(POSTGRES_DB) -c "\dt"
	@echo ""
	@echo "🎫 Event 表結構："
	@$(DEV_COMPOSE) exec db psql -U $(POSTGRES_USER) -d $(POSTGRES_DB) -c "\d \"Event\""
